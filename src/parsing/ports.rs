...
/// Generates a vector of ports based on a predefined scanning mode.
///
/// This function is a convenient way to get a port list for common scan types without
/// needing to specify them manually.
///
/// # Arguments
///
/// * `port_option` - A `PortOptions` enum variant specifying the desired mode.
///   - `PortOptions::NormalMode`: Returns ports 1-1000.
///   - `PortOptions::FastMode`: Returns the 100 most common ports.
///   - `PortOptions::SequentialMode`: Returns all ports from 1 to 65535.
///
/// # Returns
///
/// * `Ok(Vec<u16>)` containing the list of ports for the selected mode.
